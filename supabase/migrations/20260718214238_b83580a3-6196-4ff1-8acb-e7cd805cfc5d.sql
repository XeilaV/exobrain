ALTER TABLE public.note_versions
  ...
  ADD COLUMN IF NOT EXISTS linked_note_ids uuid[] NOT NULL DEFAULT '{}'::uuid[],
  ...
  ADD COLUMN IF NOT EXISTS pos_dx numeric,
  ADD COLUMN IF NOT EXISTS pos_dy numeric,
  ADD COLUMN IF NOT EXISTS icon text,
  ADD COLUMN IF NOT EXISTS event_type text NOT NULL DEFAULT 'edit',
  ADD COLUMN IF NOT EXISTS restored_from_version_id uuid;

UPDATE public.note_versions v
SET
  user_id = n.user_id,
  title = COALESCE(v.title, n.title),
  category_id = COALESCE(v.category_id, n.category_id),
  parent_note_id = COALESCE(v.parent_note_id, n.parent_note_id),
  linked_note_ids = COALESCE(v.linked_note_ids, n.linked_note_ids, '{}'::uuid[]),
  note_type = COALESCE(v.note_type, n.note_type, 'text'),
  is_collapsed = COALESCE(v.is_collapsed, n.is_collapsed, true),
  pos_dx = COALESCE(v.pos_dx, n.pos_dx),
  pos_dy = COALESCE(v.pos_dy, n.pos_dy),
  icon = COALESCE(v.icon, n.icon),
  event_type = COALESCE(v.event_type, 'edit')
FROM public.notes n
...
ALTER TABLE public.note_versions
  ...
  ALTER COLUMN title SET DEFAULT 'Nota eliminada';

ALTER TABLE public.note_versions
  DROP CONSTRAINT IF EXISTS note_versions_note_id_fkey;
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    ...
    VALUES (
      ...
      OLD.note_type, OLD.is_collapsed, OLD.pos_dx, OLD.pos_dy, OLD.icon
    );
    RETURN OLD;
  END IF;
...
DROP TRIGGER IF EXISTS note_version_snapshot ON public.notes;
...
CREATE INDEX IF NOT EXISTS idx_note_versions_user_created_at ON public.note_versions(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_note_versions_note_created_at ON public.note_versions(note_id, created_at DESC);